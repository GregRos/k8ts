...
Nodes have an `equals` method that returns true if they’re wrapping the same [[entity]]. An [[entity]] must be fully resolved to produce a [[Node]], so you can’t get a [[Node]] for a [[ForwardRef]].
