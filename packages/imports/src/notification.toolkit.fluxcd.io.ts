...
export interface AlertV1Beta2Spec {
    /**
     * EventMetadata is an optional field for adding metadata to events dispatched by the controller.
     * This can be used for enhancing the context of the event. If a field would override one already
     * present on the original event as generated by the emitter, then the override doesn't happen,
     * i.e. the original value is preserved, and an info log is printed.
     *
...
export interface ProviderV1Beta3Spec {
    ...
    /**
     * Interval at which to reconcile the Provider with its Secret references. Deprecated and not used
     * in v1beta3.
     *
...
export interface ReceiverSpec {
     ...
     * returns false then the controller will not request a reconciliation for the resource. When the
     * expression is specified the controller will parse it and mark the object as terminally failed
     * if the expression is invalid or does not return a boolean.
     *
